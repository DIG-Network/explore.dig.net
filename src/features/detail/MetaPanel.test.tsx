...
describe("<MetaPanel>", () => {
  it("renders the core facts: category, status, chain, date, author, tags", () => {
    ...
    expect(screen.getByRole("link", { name: "DIG Network" })).toHaveAttribute(
      "href",
      "https://dig.net",
    );
  ...
  it("renders license and app version only when present", () => {
    const { unmount } = renderWithIntl(
      <MetaPanel app={makeApp({ license: "MIT", version: "1.2.3" })} />,
    );
  ...
  it("renders only the extra links the listing declares", () => {
    renderWithIntl(
      <MetaPanel
        app={makeApp({ links: { docs: "https://docs.example/", discord: "https://discord.gg/x" } })}
      />,
    );
